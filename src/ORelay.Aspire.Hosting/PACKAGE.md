...
Tagged releases are published to `https://nuget.pkg.github.com/demyte/index.json`. The package is public, but GitHub's NuGet registry still requires authentication to restore it. See the repository's [version and feed instructions](https://github.com/demyte/ORelay/blob/main/docs/releases.md) for authentication and source mapping.
...
Open the relay resource in the Aspire dashboard for registration details. Each registered application has its own status, public callback URL, forward destination, last successful renewal, lease expiry, and renewal interval. Timestamps update when the AppHost acknowledges a registration or renewal. Resource relationships link applications to the relay. The relay health check summarizes all its registrations, and each application keeps its own registration health check.

The relay console logs registration, successful renewal, disconnection, registration loss, and cleanup with the application name. Application logs retain their registration diagnostics. These logs and properties exclude callback queries, OAuth state, authorization codes, and tokens. The dashboard describes registrations managed by this AppHost, not registrations belonging to other worktrees or the external server's process output.
