...
- Service updates through `autoUpdate` and `autoUpdateIntervalSeconds`.
...
The helper checks default `autoUpdate: true` and `autoUpdateIntervalSeconds: 10800`, then saves a 60-second interval and rejects 59 seconds without changing the saved value. Pass `-CheckAutoUpdate` to keep its foreground server alive beyond that interval and confirm no automatic worker result appears. It checks health after the wait, then exercises callback routing later in the same run. When checking the native worker, the helper explicitly saves `autoUpdate: false` before proving the disabled path. Focused configuration tests cover clearing both settings, defaults for missing fields in older configurations, preservation of saved `false` and custom interval values, malformed/null values, and preservation during unrelated edits. The service-manager proof is in [installation and updates](installation-updates.md).
