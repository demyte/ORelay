...
- Service updates through `autoUpdate`, `autoUpdateIntervalSeconds`, and `autoUpdateLevel`.
...
- File logging for commands that can change state, with read-only commands staying quiet. See [logging verification](relay-leases.md#console-logging).
...
The helper checks default `autoUpdate: true`, `autoUpdateIntervalSeconds: 10800`, and `autoUpdateLevel: major`, then saves a 60-second interval and rejects 59 seconds without changing the saved value. It saves each update level, rejects an invalid level, and clears the level to prove that a missing field reads as `major` without changing the file. Pass `-CheckAutoUpdate` to keep its foreground server alive beyond that interval and confirm no automatic worker result appears. It checks health after the wait, then exercises callback routing later in the same run. When checking the native worker, the helper explicitly saves `autoUpdate: false` before proving the disabled path. Focused configuration tests cover clearing settings, defaults for missing fields in older configurations, preservation of saved values, malformed/null values, case normalization, and preservation during unrelated edits. The service-manager proof is in [installation and updates](installation-updates.md).
