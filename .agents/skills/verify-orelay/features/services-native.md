...
On a matching disposable privileged host, invoke `.github/workflows/service-smoke.ps1 -ExecutablePath <absolute-native-path> -RunRoot <unique-run-directory> -Rid <win-x64-or-linux-x64> -RequireServiceProof`. It checks a conflicting definition, repeated operations, paths with spaces and a literal dollar sign, real callback delivery, registration persistence after restart, native installation against running and stopped services, preserved config, and final service removal. The x64 CI jobs require this proof; unavailable privileges or systemd fail the job after recording the prerequisite.
