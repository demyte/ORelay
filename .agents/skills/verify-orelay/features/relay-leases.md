...
The same smoke checks the file logs under `logs` beside each owned config. It verifies the startup mode, callback redirect events and secret exclusions, seeds the 2 MiB boundary, then uses real HTTP requests to rotate through four generations. Only the current file and two newest backups may survive, each no larger than 2,097,152 bytes. A separate disabled updater worker writes while the server handles requests; all expected records must remain complete and identify the actual process IDs, event IDs, service name, and disabled reason. The script copies log evidence before removing its owned state. The focused `RotatingFileLoggerProviderTests` also exercise natural size growth, UTF-8 truncation, append after reopening, concurrent writers, and recovery after an I/O failure.
