...
The same smoke checks the file logs under `logs` beside each owned config. It verifies the startup mode, callback redirect events and secret exclusions, seeds the 2 MiB boundary, then uses real HTTP requests to rotate through four generations. Only the current file and two newest log backups may survive, each no larger than 2,097,152 bytes. The empty `orelay.lock` file remains outside that log-file count. Ten separate disabled updater workers write while the server handles 100 concurrent requests; all 121 expected records, including the final rotation-trigger record, must remain complete and identify the actual process IDs, event IDs, service name, and disabled reason. The script copies log evidence before removing its owned state. The focused `RotatingFileLoggerProviderTests` also exercise natural size growth, UTF-8 truncation, append after reopening, concurrent writers, waiting for a held file lock, and recovery after an I/O failure.
...
The logging smoke also runs `cli-logging-smoke.ps1`. It verifies native configuration mutation logs, rejection without changing saved values, skipped setup, a failed start of a unique uninstalled service, and native install success, no change, and rejection of an unrelated target. All installation targets and configuration files belong to the run. It checks detailed install events, safe outcome fields, valid JSON output, raw-value/path exclusions, and read-only commands leaving logs untouched. It captures the command output, exit codes, and final log before deleting owned temporary files. It does not install a service or download an update. Focused `CliLoggingTests` and `UpdateCommandTests` cover quiet manual update checks on the managed runtime and nonfatal logging failures.
