...
`ServiceAutoUpdateService` schedules workers only when `RelayServerHost` detects a real Windows service or Linux systemd service, the executable is Native AOT, and saved `autoUpdate` is true. `autoUpdateIntervalSeconds` defaults to 86400, with a range of 60 to 2592000. Live configuration reload enables, disables, or reschedules checks. The next check is due one interval after startup or the last completed worker, and runs immediately if already overdue. The worker reads the opt-in again before calling the update engine. A foreground `server` invocation must never auto-update. Focused `ServiceAutoUpdateServiceTests` verify timer changes and worker serialization with controlled time; the native reload helper in [CLI and saved configuration](cli-config.md) proves file changes reach a running foreground relay without launching a worker.
...
Run the Windows fixture with Windows PowerShell 5.1, which compiles its small fixture executable. Repeat with `powershell.exe -NoProfile -File scripts/test-bootstrap.ps1 -Shell pwsh` to drive PowerShell 7. On Unix, run `sh scripts/test-bootstrap.sh`. These fixtures control downloads and destinations; they must reject corrupt checksums, missing assets, and releases older than `0.2.0` before installation. They also check that setup runs from the installed path, keeps config and service arguments, skips without a terminal, honors unattended defaults and skip flags, and reports install or setup failures. For a live interactive check, pipe the installer source into a shell with a real terminal. Confirm that setup reads from the terminal rather than consuming the piped source. Use only a run-owned install directory and config.

The Windows fixture also invokes the script through `Invoke-Expression` and `& ([scriptblock]::Create(...))`. Success and failure must return control to the caller without changing its error preference or leaving helper functions behind. PowerShell failures throw with the native command's exit code in the message; `powershell -File` and `pwsh -File` return `1`. The shell script retains native exit codes.
